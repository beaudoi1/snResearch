...
data=Table.read('data/Visit 8/sn2011fe-visit8-uv.flm',format='ascii')
x=data["col1"]
y=data["col2"]

#remove the absorption lines from the spectrum
nolines=np.where((x<2341) | ((x>2351) & (x<2373)) | ((x>2391) & (x<2584)) | ((x>2593) & (x<2597)) | ((x>2607) & (x<2787)) | ((x>2814) & (x<2850)) | (x>2864))
...
aa=np.array([2309,2321,2334,2360,2398,2408,2537,2684,2739,2746,2750,2763,2776,2786,2815,2862,3156])
...
plot1,=plt.plot(x,y)
plot2,=plt.plot(x,y2)
plt.savefig('data/Visit 8/b-spline.png')
plt.show()

#plots residual
plot1,=plt.plot(x,norm)
plt.ylim(0,2)
plt.savefig('data/Visit 8/residual.png')
plt.show()

#records residual plot
t=Table([x,norm],names=('col1','col2'))
t.write('data/Visit 8/residual.flm',format='ascii')